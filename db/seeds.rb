...
id = 0
RestClient::Request.execute(method: :get, url: "https://gxhz.cdht.gov.cn/api/v4/forms/86/responses", headers: {Authorization: all_authorization}).headers[:x_slp_total_pages].to_i.times do
	dates_convert_to_jsons = JSON.parse(RestClient::Request.execute(method: :get, url: "https://gxhz.cdht.gov.cn/api/v4/forms/86/responses?page=#{id+=1}", headers: {Authorization: all_authorization}))	
